...
void insertEdge(int unum, int wnum, int edgeindex) {
	Vertices* u, * w;
	IncidentEdges* new_incident, * p, *prev;
	int compare;
	...
	p = u->head;
	prev = p;
	// 간선의 순서는 삽입순서로 정렬한다. (오름차순)
	while (p->next != NULL) {
		if (p->edgeindex == -1) {
			p = p->next;
			continue;
		}
		compare = (grp.edge[p->edgeindex].endpoints[0]->num == u->num) ? 
					grp.edge[p->edgeindex].endpoints[1]->num 
					: grp.edge[p->edgeindex].endpoints[0]->num;
		if (w->num < compare) {
			new_incident->next = prev->next;
			prev->next = new_incident;
			return;
		}
		prev = p;
		p = p->next;
...
void DijkstraShortestPath(Vertices *s) {
	Vertices * u, * z;
	IncidentEdges* p;
	int sum = 0;

	for (int i = 0; i < n; i++) {													// 1. for each v E G.vertices()
		grp.vertex[i].d_label = INFINITE;											//		d(v) <- infinite value
	}
	s->d_label = 0;																	// 2. d(s) <- 0
	makeHeap();																		// 3. Q <- a priorty queue containing all ther vertices of G using d labels as Keys
	for (int i = 0; i < n; i++) {													// 4. while(!Q.isEmpty())
		u = removeMin();															//		u <- Q.removeMin()
		if (u->d_label == INFINITE)
			continue;
		p = u->head->next;
		while (p != NULL) {															//		for each e E G.incidentEdges(u)
			z = opposite(u, p);														//		z <- G.opposite(u,e)
			if (z->isLocated == true) {												//		if(z E Q.elements)
				if (u->d_label + grp.edge[p->edgeindex].weight < z->d_label) {		//			if(d(u) + w(u,z) < d(z))
					z->d_label = u->d_label + grp.edge[p->edgeindex].weight;		//				d(z) <- d(u) + w(u, z)
					replaceKey(z, z->d_label);										//				Q.replaceKey(z, d(z))
				}
			}
			p = p->next;
		}
...
int main(void) {
	buildGraph();
	DijkstraShortestPath(&grp.vertex[s-1]);
	for (int i = 1; i <= n; i++) {
		if (i == s)
			continue;
		if (grp.vertex[i - 1].d_label == INFINITE)
			continue;
		printf("%d %d\n", grp.vertex[i - 1].num, grp.vertex[i - 1].d_label);
	}
	deallocate();
}